...
export default function Navigation() {
  ...
  return (
    <nav className="bg-white/95 backdrop-blur-md shadow-xl border-b border-brand-burnt-orange/20 sticky top-0 z-50">
      <div className="container mx-auto px-responsive-4">
        <div className="flex justify-between items-center min-h-[64px] sm:h-20">
          <Link href="/" className="flex items-center text-xl font-bold text-gray-800 hover:text-gray-900 transition-all duration-300 group relative overflow-hidden">
            <div className="relative w-16 h-16 sm:w-24 sm:h-24 mr-responsive-2 sm:mr-responsive-4">
            ...
            <div className="block sm:block">
              <h3 className="text-responsive-xl md:text-responsive-2xl lg:text-responsive-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-brand-burnt-orange via-brand-vibrant-green to-brand-dark-teal leading-tight drop-shadow-sm group-hover:from-orange-500 group-hover:via-orange-400 group-hover:to-pink-500 transition-all duration-300">Emmdra</h3>
              <p className="text-responsive-sm sm:text-responsive-base font-medium text-gray-600 -mt-1">Empire & Lifestyle</p>
            </div>
          </Link>

          {/* Desktop Navigation */}
          <div className="hidden md:flex items-center gap-responsive-1">
            {[
              { name: 'Home', href: '/' },
              { name: 'Shop', href: '/shop' },
              { name: 'DIY', href: '/diy' },
            ...
            <Link
              href="/cart"
              className="group relative bg-slate-800 text-white px-responsive-4 py-responsive-2 rounded-full font-bold text-responsive-sm hover:bg-slate-900 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-110 border-2 border-slate-600 hover:border-slate-500 min-h-[44px] flex items-center gap-responsive-2"
            >
            ...
            <Link
              href="/admin"
              className="ml-responsive-4 bg-gradient-to-r from-brand-dark-teal to-brand-vibrant-green text-white px-responsive-4 py-responsive-2 rounded-lg font-semibold text-responsive-sm hover:from-brand-vibrant-green hover:to-brand-dark-teal transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 border-2 border-brand-dark-teal/50 hover:border-brand-vibrant-green/50 min-h-[44px] flex items-center"
            >
              Admin
            </Link>
          </div>
          ...
          {/* Mobile menu button */}
          <div className="md:hidden flex items-center gap-responsive-2">
            {/* Cart Indicator - Mobile */}
            <Link
              href="/cart"
              className="group relative bg-slate-800 text-white p-responsive-2 rounded-full font-bold hover:bg-slate-900 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-110 border-2 border-slate-600 hover:border-slate-500 min-h-[44px] min-w-[44px] flex items-center justify-center"
            >
              <svg className="w-5 h-5 relative z-10 transition-transform group-hover:rotate-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-2.5 5h13M12 18a2 2 0 100 4 2 2 0 000-4zm8 0a2 2 0 100 4 2 2 0 000-4z" />
              </svg>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-md border-t border-brand-burnt-orange/20 shadow-lg">
          <div className="px-responsive-2 pt-responsive-2 pb-responsive-3 space-y-1 sm:px-responsive-3">
            {[
              { name: 'Home', href: '/' },
              { name: 'Shop', href: '/shop' },
              { name: 'DIY', href: '/diy' },
            ...
            <Link
              href="/cart"
              className="group block px-responsive-3 py-responsive-3 sm:py-responsive-4 rounded-lg text-responsive-base font-bold transition-all duration-300 min-h-[48px] flex items-center transform hover:scale-105 hover:-translate-y-0.5 bg-slate-800 text-white hover:bg-slate-900 shadow-lg border-2 border-slate-600 hover:border-slate-500"
              onClick={closeMobileMenu}
            >
              <svg className="w-5 h-5 mr-responsive-3 relative z-10 transition-transform group-hover:rotate-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-2.5 5h13M12 18a2 2 0 100 4 2 2 0 000-4zm8 0a2 2 0 100 4 2 2 0 000-4z" />
              </svg>
              <span className="relative z-10 mr-responsive-2">Cart</span>
              {cartState.itemCount > 0 && (
                <span className="relative z-10 bg-red-500 text-white text-xs font-black px-responsive-2 py-1 rounded-full animate-pulse">
                  {cartState.itemCount > 99 ? '99+' : cartState.itemCount}
                </span>
              )}
            </Link>
